...
def add_additional_constraints(n):

    # Gas
    gas_cp = 0.64 # annual cp of CCGT 
    gas_i = n.generators.query('carrier == "Natural Gas"').index # get the index of all gas plants
    gas_vars = get_var(n, 'Generator', 'p')[gas_i] #get the power production from gas for all snapshots
    lhs = linexpr((1, gas_vars)).sum().sum() # sum the generation of gas --> left side of equation
    rhs = 8760 * gas_cp * get_var(n, 'Generator','p_nom')[gas_i] # maximum generation based on the optimal capacity and the maximum cp
    define_constraints(n, lhs, '<=', rhs, 'Generator', 'gas_limit') # adding the new contraint to the model

    # Biomass
    bio_cp = 0.72 # annual cp of biomass 
    bio_i = n.generators.query('carrier == "Biomass"').index 
    bio_vars = get_var(n, 'Generator', 'p')[bio_i] 
    lhs = linexpr((1, bio_vars)).sum().sum() 
    rhs = 8760 * bio_cp * get_var(n, 'Generator','p_nom')[bio_i]
    define_constraints(n, lhs, '<=', rhs, 'Generator', 'bio_limit')

    # Hydro
    # hydro_cp = 0.5 # annual cp of Hydro
    # hydro_i = n.storage_units.query('carrier == "Hydro"').index 
    # hydro_vars = get_var(n, 'Storage Unit', 'p')[hydro_i] 
    # lhs = linexpr((1, hydro_vars)).sum().sum() 
    # rhs = 8760 * hydro_cp * get_var(n, 'Storage Unit','p_nom')[hydro_i]
    # define_constraints(n, lhs, '<=', rhs, 'Storage Unit', 'hydro_limit')
